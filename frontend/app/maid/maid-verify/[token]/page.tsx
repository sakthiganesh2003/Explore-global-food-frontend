...
const VerifyInstructorEmailPage = () => {
  ...
  useEffect(() => {
    ...
    console.log(`Fetching: ${process.env.NEXT_PUBLIC_API_URL}/api/auth/verify-maid-email/${token}`);

    const verifyEmail = async () => {
      try {
        const response = await fetch(
         ` ${process.env.NEXT_PUBLIC_API_URL}/api/auth/verify-maid-email/${token}`,
          {
            method: "GET",
        ...
        if (response.ok) {
          setMessage(data.message || "maid account verified successfully!");
          setIsVerified(true);
          setTimeout(() => router.push("/"), 3000);
        } else {
          setError(data.message || "Instructor verification failed.");
        }
      } catch (err) {
