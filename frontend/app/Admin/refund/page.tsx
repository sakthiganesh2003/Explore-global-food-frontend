...
const RefundsPage = () => {
  ...
  const fetchRefunds = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('https://explorer-global-food-backend.vercel.app/api/refunds');
      if (!response.ok) {
  ...
  const handleUpdateRefundProof = async (refundId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://explorer-global-food-backend.vercel.app"}/api/refunds/proof/${refundId}`, {
        method: 'PATCH',
        body: formData,
      });
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 font-sans overflow-hidden">
      <Sidebar />
      <div className="flex-1 overflow-y-auto p-4 md:p-6 pt-20 lg:pt-8 text-gray-800 h-[100vh]">
